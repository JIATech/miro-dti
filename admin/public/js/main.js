...
function showTabletDetails(deviceId) {
  ...
  document.getElementById('tablet-details-status').textContent = tablet.online
    ? 'Online'
    : 'Offline';
  document.getElementById('tablet-details-status').className =
    `badge ${tablet.online ? 'bg-success' : 'bg-secondary'}`;
  ...
  if (tablet.hardware) {
    ...
    document.getElementById('tablet-details-device-model').textContent =
      tablet.hardware.model || 'Desconocido';
    document.getElementById('tablet-details-os-version').textContent =
      tablet.hardware.osVersion || 'Desconocido';
    document.getElementById('tablet-details-screen').textContent =
      `${tablet.hardware.screenWidth || '?'} × ${tablet.hardware.screenHeight || '?'}`;
  ...
  if (tablet.performance) {
    ...
    document.getElementById('tablet-details-battery').textContent = tablet.performance.battery
      ...
      : 'Desconocido';
    document.getElementById('tablet-details-network').textContent =
      tablet.performance.network || 'Desconocido';
  } else {
...
function updateStatsUI() {
  ...
  if (errorCountElement && appState.stats.errorsByType) {
    const totalErrors = Object.values(appState.stats.errorsByType).reduce(
      (sum, val) => sum + val,
      0
    );
    errorCountElement.textContent = totalErrors;
...
function handleDeviceResponse(data) {
  ...
  if (command === 'speak') {
    if (responseData.success) {
      ...
      showToast(
        'Error',
        `No se pudo reproducir el mensaje en ${tablet.deviceName}: ${responseData.error}`,
        'error'
      );
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    .querySelectorAll('#clear-logs-dropdown + .dropdown-menu .dropdown-item')
    .forEach((button) => {
...
function clearLogsByTimePeriod(timePeriod) {
  ...
  if (
    !confirm(`¿Está seguro que desea borrar ${confirmMessage}? Esta acción no se puede deshacer.`)
  ) {
    return;
  }
      ...
      collections.forEach((collection) => {
        ...
        if (wrapper) {
          if (appState.logs.data[collection].length === 0) {
            wrapper.innerHTML =
              '<div class="text-center text-muted p-3">No hay logs disponibles</div>';
          } else {
            updateLogDisplay();
          }
        }
