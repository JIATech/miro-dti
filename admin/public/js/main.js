...
// Importar dependencias si es necesario (para navegadores que soportan módulos)
// Si se usa en la etiqueta <script> normal, estas variables deben estar disponibles globalmente
// Estas variables se importan desde globals.js
...
function initializeUI() {
  // Inicializar elementos UI usando bootstrap (global importado de globals.js)
  ...
  // Formularios - utilizando las funciones globales importadas de globals.js
...
// Inicializar los gráficos - Chart es importado globalmente desde globals.js
...
// Conectar con Socket.IO (io importado globalmente desde socket.io.min.js)
...
// Llamar a la función updateLogDisplay importada desde globals.js
...
function handleStatsUpdate(data) {
  ...
  if (data.errorsByType) {
    ...
    if (appState.charts.errors) {
      ...
      const values = Object.values(data.errorsByType);
...
function handleTabletUpdate(data) {
  ...
  const existingIndex = appState.tablets.list.findIndex(
    (tablet) => tablet.deviceId === data.deviceId
  );
  ...
  if (
    data.status === 'online' &&
    (existingIndex < 0 || appState.tablets.list[existingIndex].status !== 'online')
  ) {
    showToast('Tablet Conectada', `${data.deviceName || data.deviceId} está en línea`, 'success');
  } else if (
    data.status === 'offline' &&
    existingIndex >= 0 &&
    appState.tablets.list[existingIndex].status !== 'offline'
  ) {
    showToast(
      'Tablet Desconectada',
      `${data.deviceName || data.deviceId} está fuera de línea`,
      'warning'
    );
  }
}

// El resto del archivo continúa con funciones adicionales...
