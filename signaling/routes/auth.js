...
router.post('/device-login', async (req, res) => {
  try {
    ...
    const hasUniqueIdentifiers =
      deviceInfo.deviceId || deviceInfo.androidId || deviceInfo.fingerprint || deviceInfo.uuid;
    ...
    console.log(
      `[AUTH] Inicio automático para ${user.username} desde dispositivo ${deviceInfo.source || 'desconocido'}`
    );
  } catch (error) {
...
async function updateDeviceInfo(user, deviceInfo) {
  try {
    ...
    if (normalizedInfo.deviceId || normalizedInfo.androidId) {
      deviceToUpdate = user.authorizedDevices.find(
        (device) =>
...
async function verifyAuthorizedDevice(deviceInfo) {
  try {
    ...
    if (!deviceInfo.username) {
      ...
      console.log(
        '[WARNING] Verificación de dispositivo sin username, usando lista predeterminada'
      );
    ...
    if (!user.authorizedDevices || user.authorizedDevices.length === 0) {
      console.log(
        `[INFO] Usuario ${user.username} sin dispositivos registrados. Usando lista predeterminada.`
      );
      return verifyAgainstDefaultList(deviceInfo);
    }
    ...
    if (deviceInfo.deviceId || deviceInfo.androidId) {
      isAuthorized = user.authorizedDevices.some(
        (device) =>
    ...
    if (!isAuthorized && deviceInfo.source === 'browser') {
      isAuthorized = user.authorizedDevices.some((device) => {
        ...
        if (deviceInfo.screenResolution && device.screenResolution === deviceInfo.screenResolution)
          matchCount++;
...
async function registerUserDevice(user, deviceInfo, req) {
  try {
    ...
    const hasUniqueIdentifiers =
      normalizedDeviceInfo.deviceId ||
      normalizedDeviceInfo.androidId ||
      normalizedDeviceInfo.fingerprint ||
      normalizedDeviceInfo.uuid;

    const isWallPanel =
      normalizedDeviceInfo.source && normalizedDeviceInfo.source.includes('wallpanel');
    ...
    if (normalizedDeviceInfo.deviceId || normalizedDeviceInfo.androidId) {
      existingDevice = user.authorizedDevices.find(
        (device) =>
    ...
    if (!existingDevice && (normalizedDeviceInfo.uuid || normalizedDeviceInfo.fingerprint)) {
      existingDevice = user.authorizedDevices.find(
        (device) =>
          ...
          (normalizedDeviceInfo.fingerprint &&
            device.fingerprint === normalizedDeviceInfo.fingerprint)
      );
    }
    ...
    if (existingDevice) {
      ...
      console.log(
        `[INFO] Dispositivo actualizado para usuario ${user.username}: ${normalizedDeviceInfo.source}`
      );
    } else {
      ...
      console.log(
        `[INFO] Nuevo dispositivo registrado para usuario ${user.username}: ${normalizedDeviceInfo.source}`
      );
    }
